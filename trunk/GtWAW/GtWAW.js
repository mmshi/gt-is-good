...
function submitCreateNewForm() {
	if($('#TBcreateNewAlias').val() == "")
		alert("Please Enter A Schedule Name");
	else
		addSchedule(1, generateString(2), $('#TBcreateNewAlias').val());
}

function clearCreateNewForm() {
	$('#TBcreateNewAlias').val("");
	var i = 0;
	var j = 0;
	for (;i<24;i++)
		for (j=0;j<7;j++)
			setTimeStatusHorrible(document.getElementById(""+i+"-"+j+"-2"));
}